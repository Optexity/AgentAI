...
export PORT=675$SLURM_ARRAY_TASK_ID

cd LLaMA-Factory
echo "Starting LLaMA API server on port $PORT"
API_PORT=$PORT llamafactory-cli api ../AgentAI/agentai/inference_configs/llama3.1_lora_sft_service_catalog_tasks_4_sub_tasks_4_seeds.yaml &
SERVER_PID=$!
...
for seed in {0..9}; do
    ...
    python main.py --seed $seed --task_num $SLURM_ARRAY_TASK_ID --port $PORT --headless --log_path ./logs/lora/service_catalog_tasks_4_sub_tasks_4_seeds/
done
